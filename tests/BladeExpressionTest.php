...
class BladeExpressionTest extends AbstractBladeTestCase
{
    ...
    {
        $this->assertSame('<html <?php echo \Diana\Support\Helpers\Emit::e($foo); ?>>', $this->compiler->compileString('<html {{ $foo }}>'));
        $this->assertSame('<html<?php echo \Diana\Support\Helpers\Emit::e($foo); ?>>', $this->compiler->compileString('<html{{ $foo }}>'));
        $this->assertSame('<html <?php echo \Diana\Support\Helpers\Emit::e($foo); ?> <?php echo app(\'translator\')->get(\'foo\'); ?>>', $this->compiler->compileString('<html {{ $foo }} @lang(\'foo\')>'));
    }
}
