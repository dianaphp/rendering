...
class BladeCustomTest extends AbstractBladeTestCase
{
    ...
    {
        $this->assertSame('<?php echo $__env->yieldContent(\'title\'); ?> - <?php echo \Diana\Support\Helpers\Emit::e(Config::get(\'site.title\')); ?>', $this->compiler->compileString("@yield('title') - {{Config::get('site.title')}}"));
    }
...
<?php endif; ?>';
    ...
    public function testCustomConditions()
    {
        ...
        $string = '@custom($user)
@endcustom';
        $expected = '<?php if ($__compiler->check(\'custom\', $user)): ?>
<?php endif; ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
...
@endcustom';
        $expected = '<?php if ($__compiler->check(\'custom\', $user)): ?>
<?php elseif ($__compiler->check(\'custom\', $product)): ?>
<?php else: ?>
<?php endif; ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
...
@elsecustom(0)
@endcustom';
        $expected = '<?php if ($__compiler->check(\'custom\', 0)): ?>
<?php elseif ($__compiler->check(\'custom\', 0)): ?>
<?php endif; ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
...
<?php echo $__env->renderComponent(); ?>';
    ...
    public function testCustomIncludes()
    {
        ...
        $string = '@input';
        $expected = '<?php echo $__env->make(\'app.includes.input\', [], \Diana\Support\Helpers\Arr::except(get_defined_vars(), [\'__data\', \'__path\'])); ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
    ...
    public function testCustomIncludesWithData()
    {
        ...
        $expected = '<?php echo $__env->make(\'app.includes.input\', [\'type\' => \'email\'], \Diana\Support\Helpers\Arr::except(get_defined_vars(), [\'__data\', \'__path\'])); ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
    ...
    public function testCustomIncludesWithExistingDirective()
    {
        ...
        $string = '@foreach';
        $expected = '<?php echo $__env->make(\'app.includes.foreach\', [], \Diana\Support\Helpers\Arr::except(get_defined_vars(), [\'__data\', \'__path\'])); ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
